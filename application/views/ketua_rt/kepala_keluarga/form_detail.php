        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body table-responsive">
                    <table class="table table-bordered table-striped">
                        <form action="<?= base_url('ketua_rt/kepala_keluarga/edit/'.$id_kk) ?>" method="post" enctype="multipart/form-data">
                            <tr>
...
<div class="modal fade" id="uploadKK<?= $id_kk ?>" tabindex="-1" role="dialog"
                    ...
                    <table class="table table-bordered table-striped">
                        <form id="uploadFoto" method="post" enctype="multipart/form-data">
                           ...
                           <td>
                                ...
                                <tr>
                                    <td>
                                        <a href="<?= base_url('ketua_rt/kepala_keluarga/detail/' . $id_kk) ?>" class="btn btn-default" id="resetBtn">Reset Form</a> &nbsp; &nbsp;
    ...
    $(document).on('submit', '#uploadFoto', function(e) {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo site_url('ketua_rt/kepala_keluarga/api_uploadKK/'.$id_kk) ?>",
            dataType: "json",
            data: form_data,
            processData: false,
            contentType: false,
...
<a href="<?= base_url('ketua_rt/kepala_keluarga') ?>" class="btn btn-default">Kembali</a>
</div>
...
<div class="modal fade" id="editAnggotaKK<?= $kk['id_anggota'] ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body table-responsive">
                    <table class="table table-bordered table-striped">
                        <form action="<?= base_url('ketua_rt/anggota/edit/'.$kk['id_anggota']) ?>" method="post" enctype="multipart/form-data">
                            <tr>
                            <input type="hidden" name="id_kk" value="<?= $kk['id_kk'] ?>" class="form-control" required="">
                            </tr>
                            <tr>
...
function hapuskk(id_kk) {
    swal({
        ...
        if (result.value) { // Only delete the data if the user clicked on the confirm button
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('ketua_rt/kepala_keluarga/api_hapus/') ?>" +
                    id_kk,
                dataType: "json",
            }).done(function() {
                swal({
                    ...
                    window.location.href = "<?php echo site_url('ketua_rt/kepala_keluarga') ?>";
                });
            }).fail(function() {
                swal({
                    title: "Gagal",
...
function hapusAnggota(id_anggota) {
    swal({
        ...
        if (result.value) { // Only delete the data if the user clicked on the confirm button
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('ketua_rt/anggota/api_hapus/') ?>" +
                    id_anggota,
                dataType: "json",
            }).done(function() {
                swal({
